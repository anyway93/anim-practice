...
## Кастомные значения spacing в Tailwind

В сборщике в конфиге Tailwind можно добавить новые значения к шкале расстояний по умолчанию так:
...
По соглашению дополнительные значения имеют вид “(цифра)px“. Цифра обозначает расстояние в px, которое конвертируется в rem. Все подобные кастомные значения обязательно возвращают расстояние в rem.


Подробнее в [вики](https://wiki.cloudmill.ru/doc/rasstoyanie-v-tailwind-4-spacing-FZBx1gZPAa/edit).
...
## Хедер: анимации, темы, управление панелями

### 1. Анимация панелей: слева налево

  По умолчанию панель появляется сверху вниз.
  Чтобы панель раскрывалась справа налево, укажи у нужного элемента атрибут:

  ```javascript
  <HeaderPanel
      data-header-panel="search"
      data-panel-animation="slide-down"
    >
    <!-- Контент панели -->
  </HeaderPanel>
  ```
 ...
 Возможные значения:
  ...
  - `slide-down` (по умолчанию) — сверху вниз
  - `slide-left` — справа налево

### 2. Смена темы хедера по секциям
Для секций, на которых требуется сменить тему хедера, указывай атрибут:

```javascript
<section data-section-color="white">...</section>
<section data-section-color="black">...</section>
<section data-section-color="custom">...</section>
```
При пересечении хедера с секцией, у элемента `.header-body` появляется модификатор:

```javascript
<header class="header-body header-body_white">
  ...
</header>
```
Класс соответствует значению из `data-section-color`.

Если открыта любая панель, модификатор-класс временно убирается. После закрытия панели класс возвращается, если он был до открытия.

### 3. Закрытие панели по кнопке
Чтобы закрыть конкретную панель через кнопку, используйте связь по атрибуту:

```javascript
<!-- Панель -->
<HeaderPanel data-header-panel="search">
    <!-- Контент панели -->
</HeaderPanel>

<!-- Кнопка закрытия этой панели -->
<ButtonSquare
  data-header-close="search"
/>
```

Атрибут значения у кнопки должен совпадать с `data-header-panel` у панели.

### 4. Взаимное закрытие dropdown и панелей
Если внутри `.header-body` есть выпадающие списки (dropdown), то:

- При открытии любой панели все dropdown автоматически закрываются.
- При открытии dropdown все панели автоматически закрываются.

Пример структуры:

```javascript
<div
  class:list={['header-body', className]}
  {...props}
>
  <Dropdown label="Годовой отчет 2024">
    <ul>
      <li>
        <a href="#">Годовой отчет 2023</a>
      </li>
      <li>
        <a href="#">Годовой отчет 2022</a>
      </li>
    </ul>
  </Dropdown>
</div>
...
```
Все эти функции работают как на десктопе, так и на мобильных устройствах.